...
```
var getPine = require('resin-pine')
var pine = getPine({
  apiUrl,
  apiVersion,
  apiKey,
...
* `dataDirectory`, string, *required in Node.js*, is the directory where the user settings are stored, normally retrieved like `require('resin-settings-client').get('dataDirectory')`.
